...
public class RecommendationSystem{
	...
	static ArrayList<Integer> genMovies = new ArrayList<Integer>();
	...
	public static void saveComputation(ArrayList<Double> prefs1, ArrayList<Double> prefs2){
		...
		if(den == 0){
			...
			// ADD the capability to put the highest values in an arrayList which will then from there be sorted and used to find the best candidates
			double r = num / den;
	 ...
	 */
		public static void movieRecommendation(){
			...
			while((line = movie.readLine()) != null && a < genMovies.size() && a < maxMovies){
				String[] movieinf = line.split(",");
				if(Integer.parseInt(movieinf[0]) == genMovies.get(a)){
